Page({
  ...
  cartLess: function(t) {
    ...
    if (t.currentTarget.dataset.type && "mch" == t.currentTarget.dataset.type) {
      ...
      a.data.mch_list[i].list[c].num = a.data.mch_list[i].list[c].num - 1, a.data.mch_list[i].list[c].price = a.data.mch_list[i].list[c].num * a.data.mch_list[i].list[c].unitPrice, a.setData({
        mch_list: a.data.mch_list
      })
    } else {
      var e = a.data.cart_list;
      for (var s in e) t.currentTarget.id == e[s].cart_id && (e[s].num = a.data.cart_list[s].num - 1, e[s].price = a.data.cart_list[s].unitPrice * e[s].num, a.setData({
        cart_list: e
      }))
    }
    a.updateTotalPrice()
  ...
  cartCheck: function(t) {
    ...
    })), "mch" == c && (a.data.mch_list[e].list[i].checked = !a.data.mch_list[e].list[i].checked, a.setData({
      mch_list: a.data.mch_list
    })), a.updateTotalPrice()
  },
  ...
  cartSubmit: function() {
    ...
    for (var l in 0 < c.length && s.push({
      ...
      if (i[l].list && i[l].list.length)
        for (var n in i[l].list) i[l].list[n].checked && (d.push(i[l].list[n].cart_id), h.push({
