...
Page({
    ...
    //     getApp().page.onLoad(this, t), is_more_comment = !(is_loading_comment = !(p = 1));
    //     share_count = 0;
    //     var e = t.user_id, a = decodeURIComponent(t.path), o = 0;
    //     if (void 0 !== e) e; else if ("undefined" == typeof my) {
    //         if (void 0 !== t.path) {
    //             o = 1;
    //             a = decodeURIComponent(t.path);
    //             var c = a.substring(30); //截取?号之后的字符串 --新增 (path=>"pages/miaosha/details/details?gid=123&uid=123")
    //             // var i = utils.scene_decode(a);
    //             var i = utils.scene_decode1(c);
    //             i.uid && i.gid ? (i.uid, t.id = i.gid) : a;
    //         }
    //     } else if (null !== getApp().query) {
    //         o = 1;
    ...
    onLoad1: function(query) { //待测试使用(改为query接收参数))
        getApp().page.onLoad(this, query), is_more_comment = !(is_loading_comment = !(p = 1));
        share_count = 0;
        var e = query.user_id, a = decodeURIComponent(query.path), o = 0;
        if (void 0 !== e) e; else if ("undefined" == typeof my) {
            if (void 0 !== query.path) {
                o = 1;
                a = decodeURIComponent(query.path);
                var c = a.substring(30); //截取?号之后的字符串 --新增 (path=>"pages/miaosha/details/details?gid=123&uid=123")
                // var i = utils.scene_decode(a);
                var i = utils.scene_decode1(c);
                i.uid && i.gid ? (i.uid, query.id = i.gid) : a;
            }
        } else if (null !== getApp().query) {
            o = 1;
            var s = getApp().query;
            getApp().query = null, query.id = s.gid;
        }
        var n = this;
        n.setData({
            id: query.id,
