...
Page({
    ...
    //     var t = this;
    //     if ("undefined" == typeof my) {
    //         var o = decodeURIComponent(e.path);
    //         if (void 0 !== o) {
    //             var c = o.substring(18); //截取?号之后的字符串 --新增 (path=>"pages/topic/topic?gid=123&uid=123")
    //             // var a = getApp().helper.scene_decode(o);
    //             var a = getApp().helper.scene_decode1(c);
    //             a.uid && a.gid && (e.id = a.gid);
    //         }
    //     } else if (null !== getApp().query) {
    //         var i = app.query;
    ...
    onLoad1: function(query) { //待测试使用(改为query接收参数))
        ...
        var t = this;
        if ("undefined" == typeof my) {
            var o = decodeURIComponent(query.path);
            if (void 0 !== o) {
                var c = o.substring(18); //截取?号之后的字符串 --新增 (path=>"pages/topic/topic?gid=123&uid=123")
                // var a = getApp().helper.scene_decode(o);
                var a = getApp().helper.scene_decode1(c);
                a.uid && a.gid && (query.id = a.gid);
            }
        } else if (null !== getApp().query) {
            var i = app.query;
            getApp().query = null, query.id = i.gid;
        }
        getApp().request({
            url: getApp().api.default.topic,
            data: {
